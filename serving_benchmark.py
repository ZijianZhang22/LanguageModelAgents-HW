...
def run_prefill(llm, token_bank, lengths, batch_size, repeats):
    ...
    for length in lengths:
        ...
        for rep in range(repeats):
            ...
            tps = input_tokens / elapsed
            rows.append({"workload":"prefill","x":length,"repeat":rep,"requests":batch_size,"input_tokens":input_tokens,"output_tokens":batch_size,"seconds":elapsed,"tokens_per_second":tps})
            print(f"prefill len={length:4d} rep={rep + 1}: {tps:,.1f} input tok/s")
    return rows
...
def run_decode(llm, token_bank, concurrencies, prompt_len, output_len, repeats):
    ...
    for n in concurrencies:
        ...
        for rep in range(repeats):
            ...
            rows.append({"workload":"decode","x":n,"repeat":rep,"requests":n,"input_tokens":prompt_len*n,"output_tokens":generated,"seconds":elapsed,"tokens_per_second":tps})
            print(f"decode n={n:2d} rep={rep + 1}: {tps:,.1f} output tok/s")
    return rows
...
def summarize(rows):
    grouped = {}
    for row in rows:
        grouped.setdefault((row["workload"], row["x"]), []).append(row["tokens_per_second"])
    out = []
