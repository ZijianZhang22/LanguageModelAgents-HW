...
def run_prefill(llm, token_bank, lengths, batch_size, repeats):
    ...
    for length in lengths:
        ...
        for rep in range(repeats):
            ...
            rows.append({"workload":"prefill","x":length,"repeat":rep,"requests":batch_size,"input_tokens":input_tokens,"output_tokens":batch_size,"seconds":elapsed,"tokens_per_second":input_tokens/elapsed})
...
def run_decode(llm, token_bank, concurrencies, prompt_len, output_len, repeats):
    ...
    for n in concurrencies:
        ...
        for rep in range(repeats):
            ...
            rows.append({"workload":"decode","x":n,"repeat":rep,"requests":n,"input_tokens":prompt_len*n,"output_tokens":generated,"seconds":elapsed,"tokens_per_second":generated/elapsed})
...
def summarize(rows):
    ...
    for (workload, x), values in sorted(grouped.items()):
        out.append({"workload":workload,"x":x,"mean_tokens_per_second":statistics.mean(values),"std_tokens_per_second":statistics.stdev(values) if len(values)>1 else 0.0,"runs":len(values)})
    return out
...
def main():
    ...
    llm = LLM(model=args.model,tensor_parallel_size=1,dtype="auto",quantization=args.quantization,kv_cache_dtype=args.kv_cache_dtype,gpu_memory_utilization=args.gpu_memory_utilization,max_model_len=args.max_model_len,enable_prefix_caching=False,trust_remote_code=True,attention_backend=args.attention_backend)
    timed_generate(llm,[make_prompt(token_bank,64)],SamplingParams(temperature=0.0,max_tokens=8,ignore_eos=True))
    rows=[]
    rows.extend(run_prefill(llm,token_bank,parse_int_list(args.prefill_lengths),args.prefill_batch,args.repeats))
    rows.extend(run_decode(llm,token_bank,parse_int_list(args.decode_concurrency),args.decode_prompt_len,args.decode_tokens,args.repeats))
    for row in rows:
